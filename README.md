# Hi there, I'm Cenk! 👋

## About Me

I'm a passionate software developer with a strong interest in open-source projects, cloud computing, and AI/ML technologies. I love exploring new technologies and contributing to the developer community.

- 🔭 Currently working on: [Akson](https://github.com/akson-ai/akson) Personal AI assistant
- 🌱 Currently learning: Machine Learning, LLMs
- 👯 Looking to collaborate on: Open-source projects, self-hosted applications
- 💬 Ask me about: Python, Go, Databases, Storage
- ⚡ Fun fact: I love playing video games!
- 💻 I use Neovim and Arch Linux, btw.

## My GitHub Stats

![Cenk's GitHub stats](https://github-readme-stats.vercel.app/api?username=cenkalti&show_icons=true&theme=radical)

## Top Languages

![Top Languages](https://github-readme-stats.vercel.app/api/top-langs/?username=cenkalti&layout=compact&theme=radical)

## Projects

Here are some of my notable projects:

- [Rain](https://github.com/cenkalti/rain): BitTorrent client in Go
- [Kuyruk](https://github.com/cenkalti/kuyruk): Simple task framework for Python
- [Dalga](https://github.com/cenkalti/dalga): MySQL backed job scheduler
- [Darbe](https://github.com/cenkalti/darbe):  RDS MySQL replication setup tool
- [PyPI Notifier](https://github.com/cenkalti/pypi-notifier): Notifies you via email about outdated packages
- [AI Writer](https://www.reddit.com/r/macapps/comments/1eqa7uy/comment/lq14476/): macOS app to rewrite selected text using LLM
- [System Guard](https://github.com/cenkalti/system-guard): Shows macOS notification if a process uses too much CPU or RAM

## Connect with Me

- [Email](mailto:cenk@alti.email)
- [LinkedIn](https://www.linkedin.com/in/cenkalti)
- [My Resume](https://github.com/cenkalti/resume)
